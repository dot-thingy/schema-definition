...
const testExamples = async (builtPath) => {
  ...
  // load examples from `.example` files to test
  const examples = listExampleFiles(processPath).reduce((acc, dirent) => {
    if (!dirent.isFile()) {
      return acc
    }
  ...
  // iterate through the examples and assert
  // that the grammar match is successful

  const errors = {}

  console.log('Beginning run of tests on examples...');

  examples.forEach(({ path, value }) => {
    console.log('  Testing example:', relative(process.cwd(), path))
    const m = myGrammar.match(value);
    try {
      if (m.failed()) {
